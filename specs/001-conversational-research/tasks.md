...
- [X] T057 [P] [US3] 添加 DialogueDecision 在结构/引用/action 校验完成前不发送答案、material clue 最多 partial 的测试到 `tests/test_dialogue.py`
- [X] T058 [P] [US3] 添加已绑定对话中的实质性新主题返回“新建对话/任务”且不创建 Action 或资产的测试到 `tests/test_dialogue.py` 与 `tests/test_conversation.py`
...
- [X] T063 [US3] 在 `src/intel_agent/models.py`、`src/intel_agent/dialogue.py` 与 `src/intel_agent/conversation.py` 增加确定性 new-topic 分流且不改绑现有 Conversation，使 T058 通过
