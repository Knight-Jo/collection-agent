...
| `fetch.py` | 执行安全网络抓取、重定向校验、HTTP 解析及文档归档。 |
...
| `search.py` | 适配 Bing、百度、百度新闻和 SearXNG，合并、排序搜索结果。 |
| `search_queries.py` | 分词、宽泛查询检测、相似度判断及六槽查询矩阵生成。 |
...
新增模块时，应保持职责单一，避免从底层模块反向导入 `agent.py` 或 `main.py`。内部调用方应直接导入定义模块，避免维护没有消费者的兼容重导出。
