...
def _append_evidence(
    cwd: Path, lines: list[str], label: str, evidence: list[EvidenceSupport]
) -> None:
    if not evidence:
        return
    lines.append("")
    ...
    for item in evidence:
        ...
        lines.append(
            f"- 来源组: {document.source_group} / {document.source_type}"
        )
        ...
        lines.append(
            f"- 引文: > {item.quote.replace(chr(10), chr(10) + '  > ')}"
        )
        if item.relation == "supports":
            review = review_for_evidence(cwd, item.id)
            lines.append(
                f"- 审核结论: {review.verdict if review else 'pending'}"
            )
            if review:
                lines.append(f"- 审核理由: {review.reason}")
                lines.append(
                    f"- 未支持部分: {'；'.join(review.unsupported_parts) or '无'}"
                )
                lines.append(
                    f"- 审核模型: {review.judge_provider}/{review.judge_model} ({review.prompt_version})"
                )
...
def generate_package(cwd: Path, task_id: str) -> dict:
    ...
    for question in task.questions:
        question_coverage = next(
            qc for qc in coverage.per_question if qc.question_id == question.id
        )
        lines.append("")
        ...
        for fact in list_active_facts_for_question(cwd, task.id, question.id):
            fact_coverage = next(
                fc for fc in question_coverage.facts if fc.fact_id == fact.id
            )
            ...
            supports = [
                ...
                and (r := review_for_evidence(cwd, e.id))
                and r.verdict == "full"
            ]
            candidates = [
                e
            ...
            lines.append(
                f"- 未消解矛盾: {fact_coverage.unresolved_conflicts + fact_coverage.unresolved_contradictions}"
            )
            _append_evidence(cwd, lines, "已验证支持证据", supports)
            _append_evidence(
                cwd, lines, "候选支持证据（不计入覆盖）", candidates
            )
            _append_evidence(cwd, lines, "反驳证据", contradicts)
    superseded = [
        f
        for f in list_facts_for_task(cwd, task.id)
    ...
    if superseded:
        ...
        for fact in superseded:
            ...
            _append_evidence(
                ...
                lines,
                "历史候选支持",
                [e for e in evidence if e.relation == "supports"],
            )
            _append_evidence(
                cwd,
                lines,
                "历史反驳证据",
                [e for e in evidence if e.relation == "contradicts"],
            )
