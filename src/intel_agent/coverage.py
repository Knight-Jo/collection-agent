...
from .storage import intel_path, read_json, sha256, verify_document_integrity, write_json_atomic
...
def _evaluate_fact(
    ...
    reviewed = [(e, review_for_evidence(cwd, e.id)) for e in candidate_supports]
    supports = [e for e, review in reviewed if review and review.verdict == "full"]
    pending = sum(1 for _, review in reviewed if review is None)
    partial = sum(1 for _, review in reviewed if review and review.verdict == "partial")
    irrelevant = sum(1 for _, review in reviewed if review and review.verdict == "irrelevant")
    contradictory_reviews = sum(1 for _, review in reviewed if review and review.verdict == "contradicts")
    ...
    high_quality_groups = {d.source_group for d in documents if d.source_type in HIGH_QUALITY}
    recent_count = sum(1 for d in documents if _is_recent(d.publish_time, now, criteria.recency_days))
    unknown_publish_time = sum(1 for d in documents if not d.publish_time or not is_valid_calendar_date(d.publish_time))
    fact_conflicts = [c for c in conflicts if c.fact_id == fact.id]
    unresolved_conflicts = sum(1 for c in fact_conflicts if c.resolution == "unresolved")
    registered = {eid for c in fact_conflicts for eid in c.evidence_ids}
    unresolved_contradictions = sum(1 for e in contradicts if e.id not in registered)
    source_gap = max(0, criteria.min_independent_sources - len(source_groups))
    quality_gap = max(0, criteria.min_high_quality_sources - len(high_quality_groups))
    recency_gap = 1 if criteria.require_recency and recent_count == 0 else 0
    gap_score = source_gap + quality_gap + recency_gap + unresolved_conflicts + unresolved_contradictions
    notes: list[str] = []
    if source_gap > 0:
...
def _evaluate_question(cwd: Path, task_id: str, question_id: str, conflicts: list[EvidenceConflict], now: datetime) -> QuestionCoverage:
    task = load_task(cwd, task_id)
    question = next(q for q in task.questions if q.id == question_id)
    facts = [_evaluate_fact(cwd, f, task.criteria, conflicts, now) for f in list_active_facts_for_question(cwd, task.id, question.id)]
...
def eval_coverage(cwd: Path, task_id: str, now: datetime | None = None) -> CoverageSnapshot:
    now = now or datetime.now(timezone.utc)
    task = load_task(cwd, task_id)
    conflicts = load_conflicts(cwd, task.id)
    per_question = [_evaluate_question(cwd, task.id, q.id, conflicts, now) for q in task.questions]
    covered = sum(1 for q in per_question if q.status == "covered")
    ratio = covered / len(task.questions)
    level = "sufficient" if ratio == 1 else "mostly_sufficient" if ratio >= 0.7 else "insufficient"
    gap_score = sum(
        (1 if q.fact_count == 0 else sum(f.gap_score for f in q.facts)) for q in per_question
    )
    fingerprint = sha256(
    ...
    no_progress_rounds = 0 if previous is None or progressed else previous.no_progress_rounds + 1
    stop_reason = "sufficient" if level == "sufficient" else ("no_progress" if no_progress_rounds >= 2 else None)
    ...
    write_json_atomic(
        ...
        CoverageHistory(task_id=task.id, snapshots=[*history.snapshots, snapshot]).model_dump(),
    )
    return snapshot
