...
def _fact_ids_for(conclusion: AssessmentConclusion) -> list[str]:
    return (
        list(dict.fromkeys(conclusion.fact_ids))
        if conclusion.kind == "inference"
        else [conclusion.fact_id]
    )


def _append_evidence(
    cwd: Path, lines: list[str], evidence: list[EvidenceSupport], label: str
) -> None:
    if not evidence:
        return
    ...
    for item in evidence:
        ...
        lines.append(
            f"  - {item.id}: {document.source_group} — {document.final_url}"
        )
        lines.append(
            f"    - 引文（行 {item.line_start}–{item.line_end}）: {item.quote.replace(chr(10), ' ')}"
        )


def generate_assessment(
    cwd: Path, task_id: str, conclusions: list[AssessmentConclusion]
) -> dict:
    ...
    for index, conclusion in enumerate(conclusions):
            ...
            errors.append(
                {
                    "index": index,
                    "code": "INVALID_INPUT",
                    "message": "结论引用了无效或跨任务的 fact_id",
                }
            )
            continue
        if any(
            len(verified_support_evidence(cwd, f.id)) == 0
            for f in conclusion_facts
            if f
        ):
            errors.append(
                {
        ...
        if conclusion.kind == "fact":
            fact_coverage = next(
                (
                    fc
                    for q in (coverage.per_question if coverage else [])
                    for fc in q.facts
                    if fc.fact_id == conclusion.fact_id
                ),
                None,
            )
            ...
            errors.append(
                {
                    "index": index,
                    "code": "INVALID_INPUT",
                    "message": "推断必须提供 statement、rationale、confidence 和 fact_ids",
                }
            )
    if errors:
    ...
    for conclusion in conclusions:
        conclusion_facts = [
            fact_by_id[fid] for fid in _fact_ids_for(conclusion)
        ]
        statement = (
            normalized_statement(conclusion.statement)
            if conclusion.kind == "inference"
            else conclusion_facts[0].statement
        )
        label = (
            "事实"
            if conclusion.kind == "fact"
        ...
        for fact in conclusion_facts:
            ...
            _append_evidence(
                cwd, lines, verified_support_evidence(cwd, fact.id), "支持证据"
            )
            _append_evidence(
                cwd,
                lines,
                [
                    e
                    for e in evidence_by_fact.get(fact.id, [])
