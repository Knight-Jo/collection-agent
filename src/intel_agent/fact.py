...
def verify_fact(
    cwd: Path, fact: Fact, ancestors: set[str] | None = None
) -> Fact:
    ancestors = ancestors or set()
    if (
        not fact.id
        or not fact.task_id
        or not fact.question_id
    ...
    if (
        ...
        or (
            ...
            and (
                not fact.superseded_by or not fact.supersession_reason.strip()
            )
        )
    ...
    if fact.status == "superseded":
        ...
        for replacement_id in fact.superseded_by:
            try:
                replacement = verify_fact(
                    cwd,
                    Fact.model_validate(
                        read_json(cwd, f"facts/{replacement_id}.json")
                    ),
                    next_ancestors,
                )
...
def load_fact(cwd: Path, fact_id: str) -> Fact:
    fact = verify_fact(
        cwd, Fact.model_validate(read_json(cwd, f"facts/{fact_id}.json"))
    )
    if fact.id != fact_id:
        raise IntelError(
            "STORAGE_CORRUPT", f"事实文件名与记录 ID 不匹配: {fact_id}"
        )
    return fact
...
def list_facts_for_task(cwd: Path, task_id: str) -> list[Fact]:
    return [
        verify_fact(cwd, Fact.model_validate(item))
        for item in list_json(cwd, "facts")
...
def list_active_facts_for_question(
    cwd: Path, task_id: str, question_id: str
) -> list[Fact]:
    return [
        f
        for f in list_active_facts_for_task(cwd, task_id)
        if f.question_id == question_id
    ]
...
def supersede_fact(
    ...
    if (
        len(replacement_ids) != len(replacement_fact_ids)
        or fact.id in replacement_ids
    ):
        raise IntelError(
            "INVALID_INPUT", "replacement facts 必须互异且不能包含原事实"
        )
    replacements = [load_fact(cwd, rid) for rid in replacement_ids]
    if any(
        r.status != "active"
        or r.task_id != fact.task_id
        or r.question_id != fact.question_id
        for r in replacements
    ):
        raise IntelError(
            "INVALID_INPUT",
            "replacement facts 必须是同任务、同问题下的活跃事实",
        )
    from .models import utc_now
