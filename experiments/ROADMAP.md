...
- [ ] **P0** crawl per-domain 配额（011 发现 etbbs 单域论坛 103/143 洪泛）+ 按问题/来源类型/域公平调度
- [ ] **P1** 来源角色修正：识别政府、企业官网/IR、新闻、研究、论坛、转载及原始发布者
- [ ] **P1** 近重复与转载血缘：同稿转载不能充当多个独立来源组
- [ ] **P1** 确定性查询矩阵：程序保留一手、独立验证、结构化数据、附件、反证和多语言查询槽位
- [ ] **P1** 语料转证据排序：按相关性、来源质量、时效、新颖性和交叉验证价值选择阅读文档
