...
export function NewTaskPage({ createRun }: { createRun: (input: RunInput) => Promise<Run> | Run }) {
      ...
      <form className="panel form-panel" onSubmit={submit}>
        <label>
          ...
          <input
            ...
            placeholder="例如：量子计算产业发展情况"
          />
        </label>
        <label>
          调研目标 <span className="field-hint">可选</span>
          <input
            aria-label="调研目标"
            value={objective}
            onChange={(event) => setObjective(event.target.value)}
            placeholder="例如：了解产业现状、政策与主要参与者"
          />
        </label>
        <fieldset>
          <legend>
            关键问题 <span>可选，最多 6 个；留空时由智能体制定</span>
          </legend>
          {questions.map((question, index) => (
            <div className="question-row" key={question}>
              <span>{index + 1}</span>
              <input
                aria-label={`关键问题 ${index + 1}`}
                value={question}
                onChange={(event) =>
                  setQuestions((items) =>
        ...
        <details className="advanced-options">
          ...
          <div className="advanced-grid">
            ...
            <label>
              ...
              <select
                aria-label="报告深度"
                value={reportDepth}
                onChange={(event) => setReportDepth(event.target.value as RunInput["report_depth"])}
              >
          ...
          <label className="crawl-toggle">
            <input
              type="checkbox"
              aria-label="启用深度抓取"
              checked={deepCrawl ?? false}
              onChange={(event) => setDeepCrawl(event.target.checked)}
            />
            启用深度抓取<span>递归收集公开关联页面和多媒体附件。</span>
          </label>
        </details>
        {error && (
        ...
        <div className="form-footer">
          <p>不同类型的信息采用相应核验标准；无法确认的内容会在报告中明确披露。</p>
          <button type="submit" className="primary-button" disabled={submitting || !systemReady}>
