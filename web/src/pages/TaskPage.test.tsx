...
it("renders crawl resource details and a task-owned download link", async () => {
  vi.spyOn(api, "task").mockResolvedValue({
    task: {
      ...
      outputs: { assessment: null, package: null },
    },
    coverage: null,
    questions: [],
    conflicts: [],
  ...
  expect(await screen.findByRole("heading", { name: "抓取资源" })).toBeInTheDocument();
...
it("shows source evidence and loads the assessment report", async () => {
  vi.spyOn(api, "task").mockResolvedValue({
    task: { id: "task-1", topic: "深度抓取", stage: "done", updated_at: "2026-08-11T09:00:00Z", criteria: { min_independent_sources: 2, min_high_quality_sources: 1, recency_days: 90, require_recency: false }, outputs: { assessment: null, package: null } },
  ...
  vi.spyOn(api, "artifact").mockResolvedValue({ kind: "assessment", path: "assessment.md", content: "# 研判结论", content_sha256: "hash" });
  ...
  await user.click(screen.getByRole("button", { name: "研判报告" }));
  expect(await screen.findByRole("heading", { name: "研判结论" })).toBeInTheDocument();
});
