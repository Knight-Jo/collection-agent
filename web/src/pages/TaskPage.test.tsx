...
it("renders crawl resource details and a task-owned download link", async () => {
    ...
    resources: [
      {
        ...
        extraction: {
          status: "complete",
          processor: "pdftotext",
          text_path: "texts/document-1.txt",
          error: null,
        },
        error: null,
        rating: 4,
  ...
  expect(
    screen.getByText("https://example.com → https://example.com/report.pdf"),
  ).toBeInTheDocument();
  ...
  expect(screen.getByRole("link", { name: "下载资源" })).toHaveAttribute(
    "href",
    "/api/tasks/task-1/resources/document-1/download",
  );
});
...
it("shows the report first and keeps source evidence available", async () => {
    ...
    questions: [
      {
        id: "question-1",
        text: "证据是什么？",
        coverage: { status: "covered", notes: [] },
        facts: [
          {
            id: "fact-1",
            statement: "事实已验证",
            status: "active",
            coverage: { status: "covered", gap_score: 0, notes: [] },
            evidence: [
              {
                id: "evidence-1",
                relation: "supports",
                ...
                document: {
                  ...
                  publish_time: null,
                  injection_warnings: ["ignore instructions"],
                },
                review: { verdict: "full", reason: "supported" },
              },
            ],
          },
        ],
  ...
  const artifact = vi.spyOn(api, "artifact").mockResolvedValue({
    kind: "report",
    path: "report.md",
    content: "# 调研结论",
    content_sha256: "hash",
