...
- `model_call`：记录模型请求序号、Token、结束原因和完整决策周期耗时。
...
- 模型请求数、输入/输出 Token、完整决策周期耗时；该耗时从本轮模型请求开始计到下一轮请求开始，包含其间的工具执行，不等同于模型服务端延迟；
- 工具调用次数、工具类型、执行状态和耗时；
- `succeeded`、`failed`、`denied`、`interrupted` 四种工具结果；
- `succeeded`、`failed`、`cancelled` 三种运行终态；
- 失败类型 `error_code`，但不保存可能含密钥的原始异常消息。

PydanticAI 成功返回工具结果只代表调用链正常；结果对象中的 `ok=false` 会记为业务失败。若返回验证错误列表，轨迹保留首个错误码和截断后的错误消息，便于识别重复失败。
...
- 技术指标：决策周期与工具耗时 P50/P95、成功率、Token 和调用分布；
