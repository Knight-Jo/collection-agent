...
- 使用非 root 用户并保持 Chromium sandbox 开启；
- 不共享宿主网络命名空间；
...
每个页面受渲染超时、请求数、下载字节和 DOM 大小限制。递归采集还受任务总字节、URL 数、深度、全局并发、单域并发和单域延迟限制。任务取消会关闭 BrowserContext；同一采集调用中的页面复用一个 Chromium 进程，结束后关闭进程。

服务器返回的主文档始终保存到 `raw_path`。JavaScript 执行后的 DOM 单独保存到 `rendered_path`，两者分别记录 SHA-256。正文和动态发现的链接来自渲染 DOM；只有正文提取完整且哈希校验通过的文档才能进入证据链。
