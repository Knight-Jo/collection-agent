# 情报调研智能体项目建设进展与下一阶段计划汇报

## ——围绕“广、深、准、用”建设本地公开信息调研能力

**汇报对象：** 项目管理层及建设决策人员  
**汇报时间：** 2026 年 8 月 31 日
...
- **阶段定位**：已达到内部试用和受控演示条件，尚未达到可直接宣布“生产验收完成”的状态。
- **工程完成度**：核心功能约完成 80%—85%；生产可靠性和量化验收约完成 65%—75%。该区间是基于能力与验收证据的工程判断，不是任务勾选比例。
- **当前主要矛盾**：已经不是“链路是否存在”，而是“搜索、交叉验证、恢复和模型行为能否在不同主题上稳定达标”。
- **建议工期**：以 1 名熟悉项目的全职工程师计算，受控试点还需约 10—15 个工作日；完成生产级验收还需约 20—28 个工程人日，即 4—6 周。若由 2 名工程师并行，可压缩到约 3—4 周，但状态存储、运行生命周期和 Agent runner 仍需串行修改。
- **暂不建设**：OpenTelemetry、PydanticAI Graph 节点轨迹、LangChain/LangGraph 迁移、多用户权限和跨任务知识库不在本轮收尾范围内。
...
2026 年 8 月 31 日对当前工作区重新执行了完整门禁：

| 检查项 | 当前结果 | 判断 |
| --- | --- | --- |
| 后端 `pytest` | 641 通过、1 跳过、2 失败 | 主链回归覆盖较完整，但当前工作区尚未全绿 |
| 后端 `pyright` | 0 error、0 warning | 类型检查通过 |
| Ruff 格式与静态检查 | 全部通过 | Python 代码规范通过 |
| Python 构建 | sdist 与 wheel 构建成功 | 包构建通过 |
| 前端单元测试 | 8 个测试文件、29 项测试全部通过 | 前端行为测试通过 |
| 前端类型、Biome 与构建 | 全部通过 | Web 工程门禁通过 |

两项后端失败均指向当前未提交的 `src/intel_agent/logging.py` 格式字符串错误：文件日志无法格式化，导致日志落盘与敏感 URL 脱敏测试失败。该问题修改量很小，预计 0.5 个工作日内可修复并重跑门禁，但在修复前不能将当前工作区标记为“全量测试通过”。

规格任务清单目前记录 74 项任务，其中 40 项勾选完成、34 项未勾选。这个 54.1% 不能代表真实功能完成度：后续提交已经实现了部分未勾选事项，例如 Task/Run 绑定、恢复矩阵、固定报告快照、前端进度和结构化轨迹。下一轮需要用 0.5—1 个工作日对 `tasks.md` 做一次事实校准，使管理口径与代码现状一致。
...
在早期实验中，模型容易用单一来源形成大量结论。当时引入事实登记门控和独立来源补证机制后，实验中的双源率由 0 提升至 86.7%，来源组由 5 增至 7，覆盖缺口分数由 25 降至 16。当前实现已将硬门控后移到 Coverage 与报告放行阶段：候选事实可继续登记，单源事实仍进入补证 backlog，避免一个难点阻塞整个主题的搜索广度。
...
| 工作包 | 当前缺口 | 预计工作量 | 完成标准 |
| --- | --- | ---: | --- |
| 工程基线恢复 | 当前日志 formatter 导致 2 项测试失败 | 0.5 天 | 后端 644 项测试全部通过或仅保留有说明的 skip |
| 运行级预算与资源释放 | 主 Agent 已统计请求和 Token，但独立审核模型调用尚未统一计入同一运行预算；`AgentDeps` 共享 HTTP client 的关闭路径仍需补齐 | 1—2 天 | 主模型、审核模型和工具请求统一记账；成功、失败、取消后 client 均关闭 |
| 生命周期故障注入 | 已有 queued/running/stopping、重试和 workspace 隔离测试，但报告取消、进程崩溃、重复调度和全恢复矩阵仍需端到端验收 | 2—3 天 | 故障后无重复消息、Run 或提交资产；所有 started 状态有终态 |
| committed-state 边界复核 | 固定 snapshot、revision manifest 和 run workspace 已实现，但需补齐所有工具的跨任务拒绝与失败路径复核 | 2—3 天 | 所有读写只见 committed snapshot 或本 Run view；失败运行无资产污染 |

P0 合计约 **5.5—8.5 个工程人日**。完成后可把当前版本作为受控环境中的候选发布版本。

### 10.2 P1：证明“广、深、准”能够稳定达标

| 工作包 | 当前缺口 | 预计工作量 | 完成标准 |
| --- | --- | ---: | --- |
| 搜索候选质量与降级 | Exa、Brave、Tavily 及国内外垂直 Provider 已接入，但真实搜索健康检查、跨查询 canonical URL 去重和实体相关性仍需专项复测 | 2—4 天 | 单点失败可降级；无明显主题污染；Must-find 召回和来源角色覆盖达标 |
| 深度与多格式验收 | PDF、图片、视频链路已打通，真实 JS、Office、音频及冲突材料样本仍不足 | 2—3 天 | 固定多格式基准在成功、失败、处理器缺失时均得到正确结果与降级 |
| 冻结配对 Benchmark | 评分器和样例已具备，但正式的 Flash/Pro 各 3 次配对运行及人工盲审尚未完成 | 3—5 天 | 6 次运行元数据完整，输出 score/compare，关键事实和引用完成复核签名 |
| 真实联网专题验收 | 当前真实运行已证明链路可用，但 SC-002、SC-003、SC-006、SC-010 尚无足量跨主题统计 | 3—5 天 | 至少覆盖政策、企业、产业、技术和事件五类主题，并形成统一指标看板 |

P1 合计约 **10—17 个工程人日**。其中真实运行会受搜索服务、模型服务和人工复核人员可用性影响，建议预留 20% 日历缓冲。

### 10.3 P2：试点后的持续优化

以下工作不阻塞受控试点，但会影响规模化使用体验：

- 专题增量更新、报告差异摘要和管理层/研究员双视图：3—5 天；
- Token、耗时和模型吞吐优化，以及本地模型策略调优：3—5 天；
- 日志与轨迹保留策略、磁盘归档和运维手册：1—2 天；
- FastAPI 生命周期弃用项和域名解析依赖弃用项升级：1—2 天；
- 任务清单、快速开始和验收记录的一致性整理：0.5—1 天。

P2 建议在试点反馈后按实际痛点排序，不建议在正式 Benchmark 之前提前扩建。

### 10.4 推荐排期

| 目标版本 | 范围 | 单人排期 | 双人排期 | 建议结论 |
| --- | --- | ---: | ---: | --- |
| 内部演示基线 | 修复日志、全量门禁、一次真实 smoke | 1—2 天 | 1 天 | 可用于内部汇报和演示 |
| 受控业务试点 | 完成 P0，并跑 2—3 个代表性专题 | 2—3 周 | 1.5—2 周 | 可邀请限定用户试用 |
| 生产级验收候选 | P0 + P1，完成配对评测和人工复核 | 4—6 周 | 3—4 周 | 可进入正式验收决策 |

以上估算基于现有单机单用户范围、代码架构不迁移、模型和搜索服务可用。若新增多用户权限、分布式运行、内部知识库、OTel 或 Graph 编排，需要另行立项，不应挤入当前收尾周期。

### 10.5 建议管理决策

1. 批准先用 1 周完成 P0 工程门禁，冻结新的架构性需求。
2. 指定 5 类代表性业务专题和人工复核人，避免评测只有技术人员自评。
3. 以“有效运行率、必查来源召回、独立验证率、引用准确率、报告人工修改率”作为试点验收指标，不以抓取网页数量或单次演示效果替代验收。
4. P1 评测完成后再决定模型采购、本地模型优化和规模化部署，不提前承诺最终模型方案。
...
- `specs/001-conversational-research/spec.md`、`plan.md` 与 `tasks.md`：目标范围、架构决策和任务台账；
- 当前 `main` 分支提交 `bfd0ab7`：L1/L2/L3 结构化可观测性与轨迹可靠性建设；
- 2026 年 8 月 31 日门禁实测：后端 pytest、pyright、Ruff、Python build，以及前端 test、typecheck、Biome、build；
