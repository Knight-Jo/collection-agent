...
def _full_pipeline(cwd):
    ...
    task = create_task(
        ...
        "测试主题",
        ["测试主题现状如何", "测试主题进展如何"],
        {"min_independent_sources": 2, "min_high_quality_sources": 1, "recency_days": 90, "require_recency": False},
    )
    ...
    assert generate_assessment(cwd, task.id, [FactConclusion(fact_id=f1.id), FactConclusion(fact_id=f2.id)])["ok"]

    set_task_stage(cwd, task.id, "challenge")
    point = {"question_ids": [q1.id], "category": "时效性", "challenge": "缺少近期来源", "gap_action": "补充近期来源"}
    ...
    confirmed = confirm_challenge(
        cwd, task.id, 1,
        [{"point_id": round1.points[0].id, "status": "addressed", "reason": "已补充", "new_evidence_ids": [new_evidence.id]}],
        [],
    )
