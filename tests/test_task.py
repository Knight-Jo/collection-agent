...
def test_create_task_validates_input(cwd):
    with pytest.raises(IntelError) as e:
        create_task(cwd, "   ", ["a"], {"min_independent_sources": 2, "min_high_quality_sources": 1, "recency_days": 90, "require_recency": False})
    assert e.value.code == "INVALID_INPUT"

    with pytest.raises(IntelError) as e:
        create_task(cwd, "主题", ["a", "b", "c", "d", "e", "f", "g"], {"min_independent_sources": 2, "min_high_quality_sources": 1, "recency_days": 90, "require_recency": False})
    assert e.value.code == "INVALID_INPUT"

    with pytest.raises(IntelError) as e:
        create_task(cwd, "主题", ["a", "b"], {"min_independent_sources": 0, "min_high_quality_sources": 1, "recency_days": 90, "require_recency": False})
    assert e.value.code == "INVALID_INPUT"


def test_create_task_dedupes_questions_and_persists(cwd):
    task = create_task(cwd, "主题", ["q1", "q1", "", "q2"], {"min_independent_sources": 2, "min_high_quality_sources": 1, "recency_days": 90, "require_recency": False})
...
def test_search_budget_exhausts(cwd):
    ...
    assert load_task(cwd, task.id).collection.search_stop_reason == "search_budget_exhausted"
...
def test_fetch_budget_resets_on_evidence(cwd):
    ...
    assert load_task(cwd, task.id).collection.stop_reason == "fetch_without_evidence"
    record_evidence_progress(cwd, task.id, 1)
    assert load_task(cwd, task.id).collection.fetch_attempts_since_evidence == 0
...
def test_done_requires_challenge_and_outputs(cwd):
    ...
    docs = [make_document(cwd, f"document {i} text about 测试主题 progress", f"https://news.cn/x{i}") for i in range(4)]
    ...
    for i, fact in enumerate([fact1, fact1, fact2, fact2]):
        save_evidence(cwd, fact.id, docs[i], "supports", f"document {i} text about 测试主题 progress")
    import asyncio
