...
def _seed_covered_task(cwd, recency_days=90, high_quality=1):
    ...
    docs = [
        make_document(cwd, "关于测试主题现状的报道 A", "https://news.cn/x1", publish_time="2026-07-01"),
        make_document(cwd, "关于测试主题现状的报道 B", "https://caixin.com/x2", publish_time="2026-07-01"),
        make_document(cwd, "关于测试主题进展的报道 C", "https://thepaper.cn/x3", publish_time="2026-07-01"),
        make_document(cwd, "关于测试主题进展的报道 D", "https://people.com.cn/x4", publish_time="2026-07-01"),
    ]
...
def test_coverage_no_progress_stops_after_two_rounds(cwd):
    task = new_task(cwd)
    q = task.questions[0]
    doc = make_document(cwd, "关于测试主题的单一来源报道", "https://news.cn/x1")
    fact = save_fact(cwd, task.id, q.id, "测试主题事实")
    save_evidence(cwd, fact.id, doc.id, "supports", "关于测试主题的单一来源报道")
    asyncio.run(audit_task_evidence(cwd, task.id, fake_judge, "test", "fake"))
