...
def _next_run_number() -> int:
    existing = [int(p.name.split("-")[0]) for p in RUNS_DIR.iterdir() if p.is_dir() and p.name.split("-")[0].isdigit()]
    return (max(existing) + 1) if existing else 1


def _git_head() -> str:
    try:
        out = subprocess.run(["git", "rev-parse", "--short", "HEAD"], capture_output=True, text=True, cwd=PROJECT_ROOT)
...
async def main() -> int:
    ...
    with open(log_path, "w", encoding="utf-8") as log:
        proc = subprocess.Popen(
            cmd, cwd=PROJECT_ROOT, env=env, stdout=log, stderr=subprocess.STDOUT
        )
        proc.wait()
