...
def analyze(run_dir: Path) -> str:
    ...
    if manifest_path.exists():
        ...
        lines.append(f"- 耗时: {m.get('elapsed_seconds')}s  exit={m.get('exit_code')}")
    ...
    for e in errors:
        pass
    failed = [e for e in trace.get("messages", []) if isinstance(e, dict) and e.get("type") == "agent" and "error" in json.dumps(e, ensure_ascii=False).lower()]
    log_path = run_dir / "run.log"
    if log_path.exists():
        log_text = log_path.read_text(encoding="utf-8")
        for m in re.finditer(r"\[(?:SEARCH_BUDGET_EXHAUSTED|COLLECTION_BUDGET_EXHAUSTED|UNSAFE_URL|NETWORK_ERROR|QUOTE_NOT_FOUND|SEMANTIC_AUDIT_FAILED|STORAGE_CORRUPT|DOCUMENT_TAMPERED|INVALID_STAGE_TRANSITION|CHALLENGE_[A-Z_]+|INVALID_INPUT|UNSUPPORTED_CONTENT|RESPONSE_TOO_LARGE)\]", log_text):
            pass
    ...
    if task:
        lines.append("## 任务最终状态")
        lines.append(f"- stage: {task.get('stage')}  challenge_round: {task.get('challenge_round')}")
        lines.append(f"- collection: {json.dumps(task.get('collection'), ensure_ascii=False)}")
        outputs = task.get("outputs", {})
        lines.append(f"- outputs: package={outputs.get('package') is not None}, assessment={outputs.get('assessment') is not None}")
        lines.append("")
    ...
    if coverage:
        ...
        if snapshots:
            last = snapshots[-1]
            lines.append("## 最新覆盖快照")
            lines.append(f"- level: {last.get('level')}  gap_score: {last.get('gap_score')}  stop_reason: {last.get('stop_reason')}")
            lines.append(f"- no_progress_rounds: {last.get('no_progress_rounds')}")
            for q in last.get("per_question", []):
                lines.append(f"- Q[{q.get('status')}] {q.get('question', '')[:40]}: facts={q.get('fact_count')} covered={q.get('covered_fact_count')}")
                for f in q.get("facts", []):
                    notes = "; ".join(f.get("notes", []))
                    lines.append(f"    - F[{f.get('status')}] gap={f.get('gap_score')} srcs={f.get('independent_sources')} hq={f.get('high_quality_sources')} recent={f.get('recent_count')} uncf={f.get('unresolved_conflicts')} {notes}")
            lines.append("")
    ...
    if conflicts and conflicts.get("items"):
        ...
        for c in conflicts["items"]:
            lines.append(f"- {c['id']}: {c['resolution']} note={c.get('note', '')[:60]}")
        lines.append("")
    if challenges and challenges.get("items"):
        for ch in challenges["items"]:
            lines.append(f"## 挑战轮次 {ch['round']}: {ch['status']} converged={ch['converged']}")
            for p in ch.get("points", []):
                lines.append(f"- [{p['status']}] {p['category']}: {p['challenge'][:60]}")
        lines.append("")

    # 事实统计
    facts_dir = run_dir / "state" / "facts"
    if facts_dir.exists():
        facts = [json.loads(f.read_text(encoding="utf-8")) for f in facts_dir.glob("*.json")]
        active = [f for f in facts if f["status"] == "active"]
        superseded = [f for f in facts if f["status"] == "superseded"]
        lines.append(f"## 事实统计: 总 {len(facts)}（active {len(active)} / superseded {len(superseded)}）")
        for f in active:
...
def _summarize_args(tool: str, args: dict) -> str:
    ...
    if tool in ("intel_status", "coverage_eval", "generate_package", "evidence_audit", "intel_assess"):
    ...
    if tool == "intel_challenge_start":
        return f"round={args.get('round')} points={len(args.get('points', []))}"
...
def _find_repeats(calls: list[dict]) -> list[str]:
    ...
    for i in range(len(seq) - 1):
        if seq[i] == seq[i + 1]:
            tool, args = seq[i]
            out.append(f"连续重复 {tool}: {_summarize_args(tool, json.loads(args) if isinstance(args, str) else args)}")
    return out
...
def main() -> int:
    ...
    parser.add_argument("--write", action="store_true", help="同时写入 ANALYSIS.md")
